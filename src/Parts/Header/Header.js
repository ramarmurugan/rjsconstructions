...
import './Header.css';
// Images
import WebsiteLogo from '../../Assets/Images/RJS-Logo.png';
    ...
    <header id='header' className={`header ${(show.change ? 'header-secondary' : '')} ${(show.header ? '' : 'hide-header')}`}>
      <nav className='header-nav container container-lg'>
        ...
        <ul className={`menu ${(show.menu ? 'show' : '')}`}>
          ...
          <li className='menu-item'>
            <Link to='/get-quote' className='menu-item-link menu-link'><span className='btn btn-primary'>get a plan</span></Link>
          </li>       
        </ul>
